...
void src::outputNeededLibraries(const std::string file, std::string librariesOutput) {
    ...
    for (size_t i = 0; i < lines.size(); ++i) {
        ...
        size_t result = -1;
        while ((result = line.find(query, result + 1)) != std::string::npos) {
            ...
            if (endPosition == std::string::npos) {
                std::cout << "Error: no \"endPosition! \"" << std::endl;
                break;
            }
...
void src::generateBuildCommand(std::string buildFlagsPath, int argc, char *argv) {
    ...
    std::string buildCommand = "g++ " + inputFile + " -lstdc++ " + allBuildFlags + " -o " + outputFileName;
#else
    std::string buildCommand = "g++ " + inputFile + " -lstdc++ " + allBuildFlags + " -o " + outputFileName;
#endif
...
void src::handleBuildingAndRunningTheProgram(std::string runLibraryInstallScripts, std::string buildFlagsPath, int argc,
                                             char *argv) {
    if (fileExists(runLibraryInstallScripts)) {
        system("sudo -S chmod -R a+rx /usr/local/share/bs/res");
        std::string cmd = "sudo -S chmod +x " + runLibraryInstallScripts + " && sudo -S bash " +
...
void src::generateInstallScript(std::string foundLibraries) {
    ...
    std::vector<std::string> availableLibrariesToBeInstalled = fk::readFileLines(
        "/usr/local/share/bs/res/availableLibs.txt");
    std::string availableLibraryBuildFlags = "/usr/local/share/bs/res/libraryBuildFlags";
    std::string buildScriptsDir = "/usr/local/share/bs/res/libraryBuildScripts/";
